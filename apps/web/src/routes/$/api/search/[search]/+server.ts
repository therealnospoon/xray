...
export async function GET({ params }: RequestEvent) {
    ...
    if (validPublicKey(search)) {
        ...
        const { publicKey = "" } = await solana.resolveBackpackUsername(
            username
        );

        console.log("Here is the publicKey on the search endpoint", publicKey);
        ...
        console.log("THIS IS THE FKN DATA", json({ data }));
